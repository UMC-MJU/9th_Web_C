...
const HomePage = () => {
  ...
  const debouncedValue = useDebounce(search, 1000); // 필요시 useDebounce(search, 300) 적용
  const [order, setOrder] = useState<"asc" | "desc">("desc");

  // Add LP 모달 상태
  const [isModalOpen, setIsModalOpen] = useState(false);

  // 무한 스크롤 훅 (order 상태를 사용하여 정렬 순서 적용)
  const {
    data: lpsPages,
    isFetching,
    hasNextPage,
    ...
    fetchNextPage,
    isError,
  } = useGetInfiniteLpList(10, debouncedValue, order === "asc" ? PAGINATION_ORDER.asc : PAGINATION_ORDER.desc);
