...
// localStorage에서 직접 값을 가져오는 헬퍼 함수
...
axiosInstance.interceptors.request.use(
  (config) => {
    // localStorage에서 직접 accessToken을 가져온다 (훅 사용 불가)
  ...
  async (error) => {
    ...
    if (
      ...
      if (originalRequest.url === `/v1/auth/refresh`) {
        // localStorage에서 직접 토큰 제거 (훅 사용 불가)
      ...
      if (!refreshPromise) {
        ...
        refreshPromise = (async () => {
          ...
          .catch((error) => {
            // 오류 발생 시 localStorage에서 직접 토큰 제거 (훅 사용 불가)
            removeLocalStorageItem(LOCAL_STORAGE_KEY.accessToken);
            removeLocalStorageItem(LOCAL_STORAGE_KEY.refreshToken);
          })
          .finally(() => {
