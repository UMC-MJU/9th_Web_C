...
function useGetLpList({ cursor, search, order, limit }: PaginationDto) {
  return useQuery({
    ...
    // 기본값을 3회 정도, 네트워크 오류 동일시적인 문제를 보완할 수 있습니다.

    // initialData: 쿼리 실행 전 미리 제공할 초기 데이터를 설정합니다.
    // 컴포넌트가 렌더링 될 때 빈 데이터 구조를 미리 제공해서, 로딩 전에도 안전하게 UI를 구성할 수 있게 해주는 애다.

    // 파라메터가 변경될 때 이전 데이터를 유지해서 UI 깜빡임 (Flicking)을 줄여줍니다.
    // ex) 페이지네이션 시 페이지 전환 사이에 이전 데이터를 보여주면서 사용자 경험을 향상시킨다.
    // keepPreviousData: true, // this keeps the previous data while fectching new data
